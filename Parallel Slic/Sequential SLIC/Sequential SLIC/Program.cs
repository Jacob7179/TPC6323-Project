...
const string OutputFolderPath = @"E:\Sequential SLIC\output";
...
using IDisposable applicationProfileSpan = EnterProfileSpan("Sequential application execution");
WriteProfileFlag("Sequential application start");
...
foreach (string inputPath in inputPaths)
{
    ...
    WriteProfileFlag("Sequential image start: " + imageName);
    using (EnterProfileSpan("Sequential image: " + imageName))
    {
        using (EnterProfileSpan("Sequential load image: " + imageName))
        {
        ...
        using (EnterProfileSpan("Sequential processing: " + imageName))
        {
            using (EnterProfileSpan("Sequential SLIC: " + imageName))
            {
                labels = RunSequentialSlic(image);
            }

            using (EnterProfileSpan("Sequential tumor detection: " + imageName))
            {
        ...
        using (EnterProfileSpan("Sequential save output: " + imageName))
        {
            SaveTumorOverlay(outputPath, image, tumorMask);
        }
    }
    WriteProfileFlag("Sequential image end: " + imageName);
...
WriteProfileFlag("Sequential application end");
...
static int[] RunSequentialSlic(ImageData image)
{
    ...
    Console.WriteLine("Sequential backend: single-thread for loop");
...
public class SequentialSlicBenchmark
{
    [Benchmark]
