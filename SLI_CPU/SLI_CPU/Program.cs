...
const float BrainRadiusFactor = 0.36f;    // Removes far outer regions. Lower = stricter against skull/skin edge.

const bool UseBrainRoi = true;            // true = only search inside the brain ROI below.
const float BrainRoiLeft = 0.40f;         // ROI left boundary as image width ratio. Lower if tumor is missed on the left.
const float BrainRoiTop = 0.04f;          // ROI top boundary as image height ratio. Increase to ignore top skull edge.
const float BrainRoiRight = 0.98f;        // ROI right boundary as image width ratio. Lower to ignore right skull edge.
const float BrainRoiBottom = 0.68f;       // ROI bottom boundary as image height ratio. Lower to ignore face/neck areas.

const string InputImagePath = @"E:\TPC SPL\Tr-gl_2.jpg"; // Put the input image path here.
const string OutputFolderPath = @"E:\SLI_CPU\output";                    // Empty = save beside input image. Or put folder path here.
