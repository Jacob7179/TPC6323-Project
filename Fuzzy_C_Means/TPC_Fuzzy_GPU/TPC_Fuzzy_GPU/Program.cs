...
const int MinFuzzyClusters = 3;
const int MaxFuzzyClusters = 5;
const int MinFuzzyIterations = 8;
const int MaxFuzzyIterations = 14;
const float FuzzyEpsilon = 0.0001f; // Prevents divide-by-zero when a pixel equals a center.
...
const string OutputFolderPath = @"E:\Fuzzy_C_Means\TPC_Fuzzy_GPU\output";
...
{
    BenchmarkRunner.Run<GpuFuzzyCMeansBenchmark>();
    return;
}

Stopwatch executionStopwatch = Stopwatch.StartNew();
string inputFolder = ResolveInputFolder(GetOptionValue(args, "--input") ?? InputFolderPath);
string outputDir = PrepareOutputFolder(GetOptionValue(args, "--output") ?? OutputFolderPath, inputFolder);
...
RunGpuFuzzyCMeans(warmupImage); // Warm-up run for ILGPU kernel compilation. Do not include this in timing.
...
{
    ImageRunResult result = ProcessImage(inputFolder, outputDir, groundTruthFolder, inputPath, RunGpuFuzzyCMeans);
...
PrintBenchmark("GPU FCM", benchmark, executionStopwatch.Elapsed.TotalMilliseconds);
PrintMseSummary("GPU FCM", mseSummary);
...
static string SaveTumorCandidateOutput(string inputFolder, string outputDir, string inputPath, ImageData image, bool[] tumorMask)
{
    ...
    string outputPath = Path.Combine(
        outputSubFolder,
        Path.GetFileNameWithoutExtension(inputPath) + "_fcm_tumor_candidate.jpg");
...
{
    using Bitmap source = new(groundTruthPath);
    using Bitmap groundTruth = new(width, height, PixelFormat.Format24bppRgb);
...
{
    Bitmap bitmap = new(source.Width, source.Height, PixelFormat.Format24bppRgb);
    using Graphics graphics = Graphics.FromImage(bitmap);
    graphics.DrawImage(source, new Rectangle(0, 0, source.Width, source.Height));
    return bitmap;
}

static int[] RunGpuFuzzyCMeans(ImageData image)
{
    int n = image.Width * image.Height;
    AutoFcmSettings autoSettings = ChooseFcmSettings(image);
    int clusterCount = Math.Clamp(autoSettings.Clusters, 2, Math.Max(2, n));
    float[] centers = InitializeFuzzyCenters(image, clusterCount);
    float[] memberships = new float[n * clusterCount];
    ...
    PrintFcmSettings(autoSettings);

    using MemoryBuffer1D<float, Stride1D.Dense> dGray = accelerator.Allocate1D(image.Gray);
    using MemoryBuffer1D<float, Stride1D.Dense> dCenters = accelerator.Allocate1D<float>(clusterCount);
    using MemoryBuffer1D<float, Stride1D.Dense> dMemberships = accelerator.Allocate1D<float>(memberships.Length);
    using MemoryBuffer1D<int, Stride1D.Dense> dLabels = accelerator.Allocate1D<int>(n);

    var kernel = accelerator.LoadAutoGroupedStreamKernel<Index1D, FuzzyCMeansViews, FuzzyCMeansSettings>(ComputeMembershipsKernel);
    FuzzyCMeansSettings settings = new(clusterCount, FuzzyEpsilon);

    for (int iter = 0; iter < autoSettings.Iterations; iter++)
    {
        dCenters.CopyFromCPU(centers);
        kernel(n, new FuzzyCMeansViews(dGray.View, dCenters.View, dMemberships.View, dLabels.View), settings);
        accelerator.Synchronize();
        dMemberships.CopyToCPU(memberships);
        UpdateFuzzyCenters(image, memberships, centers, clusterCount);
    }
...
static AutoFcmSettings ChooseFcmSettings(ImageData image)
{
    ...
    int iterations = Math.Clamp((int)MathF.Round(8f + spread / 24f), MinFuzzyIterations, MaxFuzzyIterations);
    return new AutoFcmSettings(
        Math.Clamp(clusters, MinFuzzyClusters, MaxFuzzyClusters),
        iterations,
        2.0f);
}
...
static float[] InitializeFuzzyCenters(ImageData image, int clusterCount)
{
    float min = float.MaxValue, max = float.MinValue;
    foreach (float value in image.Gray)
    {
    ...
    if (max - min <= FuzzyEpsilon)
    {
        Array.Fill(centers, min);
        return centers;
    }

    for (int i = 0; i < clusterCount; i++)
    {
        float t = clusterCount == 1 ? 0f : i / (float)(clusterCount - 1);
        centers[i] = min + (max - min) * t;
    }

    return centers;
}

static void UpdateFuzzyCenters(ImageData image, float[] memberships, float[] centers, int clusterCount)
{
    double[] weightedGray = new double[clusterCount];
    double[] weights = new double[clusterCount];

    for (int i = 0; i < image.Gray.Length; i++)
    {
        int offset = i * clusterCount;
        for (int c = 0; c < clusterCount; c++)
        {
            double membership = memberships[offset + c];
            double weight = membership * membership; // Standard FCM fuzziness m = 2.
            weightedGray[c] += weight * image.Gray[i];
            weights[c] += weight;
        }
    }
    ...
    for (int c = 0; c < clusterCount; c++)
    {
        ...
        centers[c] = (float)(weightedGray[c] / weights[c]);
    }
}
...
{
    using Bitmap output = new(image.Width, image.Height, PixelFormat.Format24bppRgb);
...
static void ComputeMembershipsKernel(Index1D pixel, FuzzyCMeansViews v, FuzzyCMeansSettings s)
{
    int i = pixel;
    int offset = i * s.ClusterCount;
    float gray = v.Gray[i];
    int exactCenter = -1;

    for (int c = 0; c < s.ClusterCount; c++)
    {
        float diff = gray - v.Centers[c];
        if (diff * diff <= s.Epsilon)
        {
            exactCenter = c;
            break;
        }
    ...
    {
        for (int c = 0; c < s.ClusterCount; c++)
            v.Memberships[offset + c] = c == exactCenter ? 1f : 0f;

        v.Labels[i] = exactCenter;
        return;
    }

    float bestMembership = -1f;
    int bestLabel = 0;

    for (int c = 0; c < s.ClusterCount; c++)
    {
        float diffC = gray - v.Centers[c];
        float distanceC = diffC * diffC + s.Epsilon;
        float denominator = 0f;

        for (int k = 0; k < s.ClusterCount; k++)
        {
            float diffK = gray - v.Centers[k];
            float distanceK = diffK * diffK + s.Epsilon;
            denominator += distanceC / distanceK; // Standard FCM membership for fuzziness m = 2.
        }

        float membership = 1f / denominator;
        v.Memberships[offset + c] = membership;
...
public class GpuFuzzyCMeansBenchmark
{
    [Benchmark]
...
public readonly record struct FuzzyCMeansViews(
    ArrayView<float> Gray,
    ArrayView<float> Centers,
    ArrayView<float> Memberships,
    ArrayView<int> Labels);

public readonly record struct FuzzyCMeansSettings(int ClusterCount, float Epsilon);
